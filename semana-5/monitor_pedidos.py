"""
PROYECTO: EcoMarket - Monitor de Inventario (Hito 1)

RESUMEN DE DECISIONES DE DISEÑO:
1. TIMEOUT: 10s -> Trade-off: Tolerancia a redes lentas vs. Tiempo de bloqueo del cliente. 
   Decisión: Evita que el cliente quede colgado infinitamente si el API no responde, 
   dando un margen razonable para conexiones móviles inestables.

2. BACKOFF EXPONENCIAL: Base 5s hasta Max 60s -> Trade-off: Recuperación lenta vs. Cortesía 
   con el servidor. Decisión: Implementado solo en errores 5xx y timeouts para no 
   bombardear a EcoMarket cuando su servidor ya está sufriendo.

3. ETAG / IF-NONE-MATCH: Manejo de estado persistente -> Trade-off: Complejidad en headers 
   vs. Ahorro de procesamiento. Decisión: Crucial para recibir 304 (Sin Cambios) y evitar 
   re-procesar datos que los observadores ya conocen.

4. PATRÓN OBSERVER: Desacoplamiento de módulos -> Trade-off: Abstracción vs. Simplicidad. 
   Decisión: Permite que 'ModuloCompras' y 'ModuloAlertas' funcionen de forma 
   independiente; si uno falla, el Monitor y los demás siguen vivos.

5. POLLING ADAPTATIVO: Incremento por inactividad -> Trade-off: Latencia vs. Eficiencia. 
   Decisión: Si tras varios ciclos no hay cambios, el cliente baja la frecuencia de 
   muestreo automáticamente para ahorrar recursos y batería.
"""
